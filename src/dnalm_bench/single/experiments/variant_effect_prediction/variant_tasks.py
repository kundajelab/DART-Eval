...
def load_embeddings_and_compute_cosine_distance(embedding_dir, progress_bar=False):
    '''
    ...
    for i, file in enumerate(emb_files):
        ...
        for key in tqdm(sorted_list, disable = (not progress_bar)):
            ...
            if "idx_var" in list(file['allele1'].keys()):
                ...
                for i in range(allele1_array.shape[0]):
                    a = np.mean(allele1_array[i, :, :], axis=0) # average over the tokens for each variant (result = batch_size x embedding_size)
                    b = np.mean(allele2_array[i, :, :], axis=0) 
                    cosine_distances+=[spatial.distance.cosine(a, b)]
